...
class VestManager: NSObject {
  ...
  static func vestInfo(completion: @escaping ((_ info: VestInfo?) -> Void)) {
    ...
    HttpClient.requestObject(method: .get, url: vestUrl, parameters: parameters, loading: false) { (result: VestInfo?) in
      completion(result)
    }
  }
